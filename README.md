...
My Solution: SELECT state, SUM(user_id) FROM usersAddress GROUP BY state

2. Find the most popular area code in the `usersContact` table.

- Hint: SUBSTR, GROUP BY

My Solution: SELECT SUBSTRING(phone1, 1, 3) AS areaCode, COUNT(\*) total FROM usersContact GROUP BY areaCode ORDER BY total DESC

3. Find the MIN first_name, the county, and a count of all users in that county for counties with more than 10 users. There will be four results. List the last one.

- Hint: MIN, COUNT, JOIN, GROUP BY, HAVING

My Solution: SELECT MIN(first_name), county, count(user_id) AS countyTotal FROM usersAddress JOIN users WHERE users.id = usersAddress.user_id GROUP BY county HAVING countyTotal > 10
